...
@Service
public class WalletaddressServiceImpl extends BaseServiceImpl implements WalletaddressService  {
	...
	public ModelAndView getPaging(Integer page,String where){ 
		...
		try{
			...
			if(allRows<=0){
				...
				mv.addObject("list", new ArrayList<Walletaddress>());
				return mv;
			}
			...
			List<Walletaddress> list=mapper.getList(parkSql);
			if(list==null||list.size()<=0)
				list=new ArrayList<Walletaddress>();
	...
	public boolean isExist(java.lang.Integer id){
		Walletaddress ad=mapper.getById(id);
		if(ad==null){
			return false;		
		}else{
			return true;
