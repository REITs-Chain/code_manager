...
@Service
public class UserRiskServiceImpl extends BaseServiceImpl implements UserRiskService  {
	
	@Autowired
	private UserRiskMapper mapper;
	...
	public ModelAndView getPaging(Integer page,String where){ 
		...
		try{
			...
			if(allRows<=0){
				...
				mv.addObject("list", new ArrayList<UserRisk>());
				return mv;
			}
			...
			List<UserRisk> list=mapper.getList(parkSql);
			if(list==null||list.size()<=0)
				list=new ArrayList<UserRisk>();
