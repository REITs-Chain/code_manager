...
@Service
public class RawassetServiceImpl extends BaseServiceImpl implements RawassetService  {
	
	@Autowired
	private RawassetMapper mapper;
	...
	public ModelAndView getPaging(Integer page,String where){ 
		...
		try{
			...
			if(allRows<=0){
				...
				mv.addObject("list", new ArrayList<Rawasset>());
				return mv;
			}
			...
			List<Rawasset> list=mapper.getList(parkSql);
			if(list==null||list.size()<=0)
				list=new ArrayList<Rawasset>();
