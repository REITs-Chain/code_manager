...
@Service
public class GroupApplyServiceImpl extends BaseServiceImpl implements GroupApplyService  {
	
	@Autowired
	private GroupApplyMapper mapper;
	...
	public ModelAndView getPaging(Integer page,String where){ 
		...
		try{
			...
			if(allRows<=0){
				...
				mv.addObject("list", new ArrayList<GroupApply>());
				return mv;
			}
			...
			List<GroupApply> list=mapper.getList(parkSql);
			if(list==null||list.size()<=0)
				list=new ArrayList<GroupApply>();
	...
	public boolean isExist(java.lang.Integer id){
		GroupApply ad=mapper.getById(id);
		if(ad==null){
			return false;		
		}else{
			return true;
