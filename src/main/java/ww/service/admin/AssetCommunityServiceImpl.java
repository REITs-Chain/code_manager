...
@Service
public class AssetCommunityServiceImpl extends BaseServiceImpl implements AssetCommunityService  {
	
	@Autowired
	private AssetCommunityMapper mapper;
	...
	public ModelAndView getPaging(Integer page,String where){ 
		...
		try{
			...
			if(allRows<=0){
				...
				mv.addObject("list", new ArrayList<AssetCommunity>());
				return mv;
			}
			...
			List<AssetCommunity> list=mapper.getList(parkSql);
			if(list==null||list.size()<=0)
				list=new ArrayList<AssetCommunity>();
	...
	public boolean isExist(java.lang.Integer id){
		AssetCommunity ad=mapper.getById(id);
		if(ad==null){
			return false;		
		}else{
			return true;
