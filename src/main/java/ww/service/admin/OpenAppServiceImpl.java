...
@Service
public class OpenAppServiceImpl extends BaseServiceImpl implements OpenAppService  {
	
	@Autowired
	private OpenAppMapper mapper;
	...
	public ModelAndView getPaging(Integer page,String where){ 
		...
		try{
			...
			if(allRows<=0){
				...
				mv.addObject("list", new ArrayList<OpenApp>());
				return mv;
			}
			...
			List<OpenApp> list=mapper.getList(parkSql);
			if(list==null||list.size()<=0)
				list=new ArrayList<OpenApp>();
