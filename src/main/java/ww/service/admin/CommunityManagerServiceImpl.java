...
@Service
public class CommunityManagerServiceImpl extends BaseServiceImpl implements CommunityManagerService  {
	
	@Autowired
	private CommunityManagerMapper mapper;
	...
	public ModelAndView getPaging(Integer page,String where){ 
		...
		try{
			...
			if(allRows<=0){
				...
				mv.addObject("list", new ArrayList<CommunityManager>());
				return mv;
			}
			...
			List<CommunityManager> list=mapper.getList(parkSql);
			if(list==null||list.size()<=0)
				list=new ArrayList<CommunityManager>();
	...
	public boolean isExist(java.lang.Integer id){
		CommunityManager ad=mapper.getById(id);
		if(ad==null){
			return false;		
		}else{
			return true;
