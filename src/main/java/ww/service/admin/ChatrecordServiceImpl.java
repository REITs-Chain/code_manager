...
@Service
public class ChatrecordServiceImpl extends BaseServiceImpl implements ChatrecordService  {
	...
	public ModelAndView getPaging(Integer page,String where){ 
		...
		try{
			...
			if(allRows<=0){
				...
				mv.addObject("list", new ArrayList<Chatrecord>());
				return mv;
			}
			...
			List<Chatrecord> list=mapper.getList(parkSql);
	...
	public boolean isExist(java.lang.Integer id){
		Chatrecord ad=mapper.getById(id);
		if(ad==null){
			return false;		
		}else{
			return true;
