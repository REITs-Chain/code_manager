...
@Service
public class TxServiceImpl extends BaseServiceImpl implements TxService  {
	
	@Autowired
	private TxMapper mapper;
	...
	public ModelAndView getPaging(Integer page,String where){ 
		...
		try{
			...
			if(allRows<=0){
				...
				mv.addObject("list", new ArrayList<Tx>());
				return mv;
			}
			...
			List<Tx> list=mapper.getList(parkSql);
			if(list==null||list.size()<=0)
				list=new ArrayList<Tx>();
