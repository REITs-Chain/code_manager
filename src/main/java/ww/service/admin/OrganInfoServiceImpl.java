...
import ww.dao.OrganInfoMapper;

@Service
public class OrganInfoServiceImpl extends BaseServiceImpl implements OrganInfoService  {
	
	@Autowired
	private OrganInfoMapper mapper;
	
	public OrganInfoMapper getMapper(){
		return mapper;
	}
	...
	public ModelAndView getPaging(Integer page,String where){ 
		...
		try{
			...
			if(allRows<=0){
				...
				mv.addObject("list", new ArrayList<OrganInfo>());
				return mv;
			}
			...
			List<OrganInfo> list=mapper.getList(parkSql);
	...
	public boolean isExist(java.lang.Long id){
		OrganInfo ad=mapper.getById(id);
		if(ad==null){
			return false;		
		}else{
			return true;
	...
	@Transactional
	public boolean Save(OrganInfo data){	
	    if(data==null)
			return false;
		try{
